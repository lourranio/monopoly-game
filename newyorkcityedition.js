...
function citytax() {
	...
	if (p.human) {
		...
		addalert(p.name + " pagou $ 200 de imposto municipal.");
		p.pay(200, 0);
	}
}

function luxurytax() {
	addalert(player[turn].name + " pagou $ 75 de Imposto de Luxo.");
...
square[0] = new Square("GO", "Receba $ 200 DE SALÁRIO À MEDIDA QUE VOCÊ PASSA.", "white");
...
square[30] = new Square("Va para a cadeia", "Go directly to Jail. Do not pass GO. Do not collect $200.", "white");
...
communityChestCards[0] = new Card("Sair da cadeia livre. Este cartão pode ser guardado até que seja necessário ou vendido.", function () { p.communityChestJailCard = true; updateOwned(); });
